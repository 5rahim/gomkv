...
func (p *MediaParser) HandleString(id gomkv.ElementID, value string, info gomkv.ElementInfo) error {
	switch id {
...
func (p *MediaParser) HandleInteger(id gomkv.ElementID, value int64, info gomkv.ElementInfo) error {
	switch id {
	...
	case gomkv.TrackTypeElement:
		switch value {
		case gomkv.TrackType_Video:
			p.hasVideo = true
		case gomkv.TrackType_Audio:
			p.hasAudio = true
		}
	}
	return nil
...
func (p *MediaParser) HandleFloat(id gomkv.ElementID, value float64, info gomkv.ElementInfo) error {
	switch id {
...
func parseFile(path string) (*MediaFile, error) {
	...
	coverh := gomkv.CoverHandler{}
	err = gomkv.ParseSections(file, gomkv.NewHandlerChain(tagsh, &coverh, &handler), gomkv.InfoElement, gomkv.TagsElement, gomkv.TracksElement, gomkv.AttachmentsElement)
	if err != nil {
		return nil, err
	}
	...
	mf := MediaFile{
		Title:             handler.title,
		Artist:            tagsh.Tags()[gomkv.Tag_Artist],
		Cover:             coverh.Data,
				...
				if err != nil {
					...
					if len(file.Cover) > 0 {
						...
						if _, err := os.Stat(thumbFile); os.IsNotExist(err) {
							...
							if err := os.WriteFile(thumbFile, img, 0644); err != nil {
								return err
							}
						}
